...
которые попадают в папку (по умолчанию drop/).
"""
...
from pipeline_logger import get_logger, setup_logging
from webm_to_txt import SUPPORTED_EXTENSIONS

log = get_logger("watch")

# Файл считается "готовым", если не менялся последние N секунд
STABILITY_SEC = 3
POLL_INTERVAL = 2


def watch_drop_dir(drop_dir: Path, language: str = "ru", move_after: bool = True):
    """
    Опрашивает drop_dir и обрабатывает новые файлы.

    Args:
        drop_dir: Директория для наблюдения
        language: Язык аудио
        move_after: Перемещать обработанные в drop/processed/
    """
    ...
    while True:
        try:
            for f in drop_dir.iterdir():
                ...
                # Ждём стабилизации (файл не копируется)
